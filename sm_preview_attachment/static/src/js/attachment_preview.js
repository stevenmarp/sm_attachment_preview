...
// Try to import showExcelPreviewModal (only available when report_preview.js exists)
let showExcelPreviewModal = null;
try {
    const reportModule = await import("./report_preview");
    showExcelPreviewModal = reportModule.showExcelPreviewModal;
} catch {
    // report_preview.js not available — Excel preview will fallback to Office Online / download
}
...
patch(BinaryField.prototype, {
    ...
    smOpenPreview(ev) {
        ...
        if (previewType === 'excel' || isOffice) {
            if (previewType === 'excel' && showExcelPreviewModal) {
            ...
            fetch(contentUrl, { method: 'HEAD' }).then(resp => {
                ...
                if (detectedType === 'other') {
                    ...
                    if (showExcelPreviewModal) {
                        ...
                        showPreviewModal('excel', contentUrl, fileName, downloadUrl, true);
                    }
    ...
    smOpenAttachmentPreview(ev, file) {
        ...
        if (previewType === 'excel') {
            if (showExcelPreviewModal) {
                ...
                showPreviewModal('excel', contentUrl, file.name, downloadUrl, true);
            }
            return;
        }
